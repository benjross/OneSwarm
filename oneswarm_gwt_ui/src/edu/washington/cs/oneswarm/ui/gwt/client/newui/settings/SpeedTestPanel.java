...
public class SpeedTestPanel extends VerticalPanel {
    ...
    private void updateUpload() {
        ...
        double frac = Double.parseDouble(fractionSelection.getValue(fractionSelection
                .getSelectedIndex()));
    ...
    public SpeedTestPanel(final SpeedTestCallback cb) {
        ...
        updateTimer = new Timer() {
            @Override
            public void run() {
                OneSwarmRPCClient.getService().getBackendTask(OneSwarmRPCClient.getSessionID(),
                        task.getTaskID(), new AsyncCallback<BackendTask>() {
        ...
        OneSwarmRPCClient.getService().performSpeedCheck(OneSwarmRPCClient.getSessionID(), 0,
